...
class Ui_CreateReceipt(object):
    ...
    def __init__(self,medlist, OrderForm):
        self.medlist = medlist
        self.orderform = OrderForm

        self.CreateReceipt =QtWidgets.QMainWindow()
        self.setupUi(self.CreateReceipt)
        self.CreateReceipt.show()
    ...
    def retranslateUi(self, CreateReceipt):
        ...
        self.pushButton.clicked.connect(lambda: self.proceedLogin(CreateReceipt))


        self.totalprice = 0;
        for med in self.medlist:
            self.plainTextEdit.setPlainText(med.name)
            self.totalprice += med.totalprice
            
        self.lcdNumber.display(self.totalprice)
        
    def proceedLogin(self, CreateReceipt):
        CreateReceipt.close()
