...
from PaymentReceipt import Ui_CreateReceipt
...
class Ui_MainWindow(object):
    ...
    def retranslateUi(self, MainWindow):
        ...
        self.pushButton.clicked.connect(lambda: self.orderMed(MainWindow))

        
    def orderMed(self,MainWindow):
        idMed = self.lineEdit.text()
        stock = self.spinBox.value()

        medController = MedicineController()
        medlist = medController.orderMedicine(idMed, stock)
        
        MainWindow.close()
        self.receipt = Ui_CreateReceipt(medlist,self)
